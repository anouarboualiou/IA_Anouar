...
export function getCastByMovieId(movieId){
    ...
    const roles = ROLES.filter(rol => rol.movieId === movieId)

    const cast = roles.map(rol => {

        const actores = ACTORS.find(actor => actor.id === rol.actorId)
...
export function getFilmographyByActorId(actorId){
    ...
    const roles = ROLES.filter(rol => rol.actorId === actorId)

    const filmografia = roles.map(rol => {
        return MOVIES.find(movie => movie.id === rol.movieId)
    })
